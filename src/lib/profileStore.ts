...
const KEY_LAST_ROUTE           = 'bw.last.route';  // NEW: remember last visited route

/* Notifications */
function notifyProfileChanged(key?: string, value?: string) {
  try {
    window.dispatchEvent(new Event('bw:profile:update'));
    if (key) {
      window.dispatchEvent(new StorageEvent('storage', { key, newValue: value } as StorageEventInit));
    }
  } catch {}
}
function writeLegacyCoordsMirror(p: Profile) {
  try {
    if (p && typeof p.lat === 'number' && typeof p.lng === 'number') {
      localStorage.setItem(KEY_LEGACY_COORDS_MIRROR, JSON.stringify({ lat: p.lat, lng: p.lng }));
    }
  } catch {}
}
...
export function getActivePhone(): string | null {
  try { const s = sessionStorage.getItem(KEY_ACTIVE_PHONE_SESSION); if (s) return s; } catch {}
  try { const p = localStorage.getItem(KEY_ACTIVE_PHONE_PERSIST); return p || null; } catch { return null; }
}
...
export function upsertUser(patch: Profile) {
  ...
  const existing = loadProfileByPhone(patch.phone) || { phone: patch.phone };
...
export function removeProfile(phone: string) {
  try {
    ...
    notifyProfileChanged(key, null as unknown as string);
    ...
    if (active === phone) {
      ...
      notifyProfileChanged(KEY_ACTIVE_PHONE_SESSION, null as unknown as string);
    }
  } catch {}
}
export function saveLabeledAddress(phone: string, next: { label: string; line?: string; lat?: number; lng?: number }) {
...
export function hasUser(phone: string): boolean {
  try { return Boolean(localStorage.getItem(KEY_PROFILE_PREFIX + phone)); } catch { return false; }
}
...
export function ensureActiveProfile(): Profile | null {
  ...
  if (all.length) { setActivePhone(all[0].phone); return all[0]; }
  return null;
}

/* last route helpers (for post-login return) */
export function setLastRoute(path: string) {
  try { localStorage.setItem(KEY_LAST_ROUTE, path); } catch {}
}
export function getLastRoute(): string | null {
  try { return localStorage.getItem(KEY_LAST_ROUTE) || null; } catch { return null; }
}
