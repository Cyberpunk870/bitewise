...
import { decidePerm, setPermPolicy, allowForThisSession, type PermPolicy } from '../../lib/permPrefs';
...
function RotatingDishImage({ names, fallback, intervalMs = 3000 }:{
...
function Tile({
  ...
  return (
    <div className="relative rounded-2xl border bg-white shadow-sm overflow-hidden">
      ...
      {both && (
        <button className="absolute right-3 top-3 text-xs px-3 py-1 rounded-full bg-black text-white"
          onClick={() => onCompare(r)} aria-label={`Compare ${r.name}`}>Compare</button>
      )}
      {typeof price === 'number' && (
...
export default function Availability() {
  ...
  // permission guard
  const [perm, setPerm] = useState(decidePerm('location'));
  useEffect(() => {
    if (perm === 'deny') {
      addNotice({
        kind: 'system',
    ...
    <main className="min-h-screen bg-gradient-to-br from-pink-500 to-orange-400 pb-16">
      <div className="max-w-4xl mx-auto w-full px-3 pt-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <button className="px-3 py-1.5 text-sm rounded-full border bg-white/80" onClick={() => nav(-1)}>
            ← Back
          </button>
        ...
        {/* permission blocker */}
        {perm === 'deny' ? (
          <LocationBlocked onChoose={handlePermChoice} />
        ) : (
          <>
            {/* filters */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
              <div className="rounded-2xl border bg-white/80 p-3">
                ...
                <input type="range" min={50} max={1500} value={priceMax} onChange={(e) => setPriceMax(Number(e.target.value))} className="w-full" />
              </div>
              <div className="rounded-2xl border bg-white/80 p-3">
                <div className="text-xs opacity-70 mb-1">Rating ≥ {ratingMin.toFixed(1)}</div>
                <input type="range" min={0} max={5} step={0.5} value={ratingMin} onChange={(e) => setRatingMin(Number(e.target.value))} className="w-full" />
              </div>
              <div className="rounded-2xl border bg-white/80 p-3">
                <div className="text-xs opacity-70 mb-1">Distance ≤ {distanceMax} km</div>
                <input type="range" min={1} max={20} value={distanceMax} onChange={(e) => setDistanceMax(Number(e.target.value))} className="w-full" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {rows.map(({ r, matches }) => (
                <Tile key={r.id} r={r} matchedDishNames={matches} onCompare={(row) => nav(`/compare/${row.id}`)} />
              ))}
            </div>
          </>
        )}
