...
export default function AddressPick() {
  ...
  // Input + coords (seed with any stored value)
  const [input, setInput] = useState(storedLine ?? '');
  ...
  // NEW: readiness + queued action (fixes the “two clicks” issue)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const center: Coords = coords ?? { lat: 28.6139, lng: 77.209 }; // Delhi fallback
      ...
      // mark ready and replay queued "use my location" if user clicked early
  ...
  const useMyLocation = useCallback(() => {
    ...
    const run = () => {
      ...
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          await movePin(p, true); // also reverse‑geocode
          setBusy(false);
        },
        (err) => {
          setBusy(false);
    ...
    // If map/marker not ready yet, queue the action so the FIRST click still works.
    if (!mapReady.current) {
  ...
  const handleConfirm = () => {
    ...
    nav('/onboarding/address/label', { replace: true }); // ensure correct route
  };
