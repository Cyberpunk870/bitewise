...
const alert_1 = require("../lib/alert");
const firebaseAdmin_1 = require("../lib/firebaseAdmin");
const verifyAuth_1 = require("../middleware/verifyAuth");
const router = (0, express_1.Router)();
const log = logger_1.default.child({ module: "webauthn" });
...
const DEFAULT_RP_ID = process.env.WEBAUTHN_RP_ID ||
...
const originEnv = process.env.WEBAUTHN_ALLOWED_ORIGINS ||
...
function originsForRequest(req) {
    const host = hostFromReq(req);
    const dynamic = host ? [`https://${host}`] : [];
    if (!EXPECTED_ORIGINS.length)
        return dynamic.length ? dynamic : ["http://localhost:5173"];
...
function ensureAdminReady() {
    return withTimeout(Promise.resolve().then(() => (0, firebaseAdmin_1.ensureAdmin)()), ADMIN_TIMEOUT_MS, "ensureAdmin");
}
...
async function ensureUserForPhone(phone) {
    ...
    const snap = await withDbTimeout(db.collection("users").where("phone", "==", phone).limit(1).get(), "findUserForPhone");
    if (snap.empty)
        return null;
    const doc = snap.docs[0];
    return { uid: doc.id, data: doc.data() || {} };
}
async function getPasskeys(uid) {
    const db = (0, firestore_1.getFirestore)();
    const snap = await withDbTimeout(db.collection("users").doc(uid).collection(PASSKEY_COLLECTION).get(), "getPasskeys");
    return snap.docs.map((doc) => ({ ...doc.data(), id: doc.id }));
}
async function setChallenge(uid, type, challenge) {
    const db = (0, firestore_1.getFirestore)();
    const now = new Date().toISOString();
    await withDbTimeout(db
        .collection("users")
        .doc(uid)
        .set({
    ...
    }, { merge: true }), "setChallenge");
}
async function getChallenge(uid) {
    const db = (0, firestore_1.getFirestore)();
    ...
    const type = data.webauthnChallengeType;
...
async function clearChallenge(uid) {
    ...
    }, { merge: true }), "clearChallenge");
}
function challengeFresh(ts) {
    if (!ts)
        return false;
    const age = Date.now() - new Date(ts).getTime();
    return age >= 0 && age <= CHALLENGE_MAX_AGE_MS;
}
function toCredentialDescriptor(pk) {
    return {
        id: pk.credentialId,
...
async function logWebauthnEvent(name, meta = {}) {
    try {
        await (0, firestore_1.getFirestore)().collection("analytics_events").add({
            name,
            ts: Date.now(),
            meta,
        });
...
function passkeyResponse(pkDoc) {
    return {
        id: pkDoc.id,
...
router.get("/passkeys", verifyAuth_1.verifyAuth, async (req, res) => {
    ...
    try {
        ...
        await withTimeout(Promise.resolve().then(() => (0, firebaseAdmin_1.ensureAdmin)()), timeoutMs, "ensureAdmin");
        ...
        log.info({ uid, route: "passkeys", count: sorted.length, ms: Date.now() - started }, "passkeys ok");
        return res.json({ ok: true, passkeys: sorted.map(passkeyResponse) });
    }
    catch (err) {
        log.error({ err, route: "passkeys", ms: Date.now() - started }, "GET /passkeys failed");
...
router.delete("/passkeys/:id", verifyAuth_1.verifyAuth, async (req, res) => {
    try {
        const uid = req.user?.uid || req.uid;
        if (!uid)
            return res.status(401).json({ ok: false, error: "unauthorized" });
        ...
        await ensureAdminReady();
        const db = (0, firestore_1.getFirestore)();
        await withDbTimeout(db.collection("users").doc(uid).collection(PASSKEY_COLLECTION).doc(id).delete(), "deletePasskey");
...
router.post("/register/options", verifyAuth_1.verifyAuth, async (req, res) => {
    const started = Date.now();
    try {
        const timeoutMs = 7000;
        const uid = req.user?.uid || req.uid;
        ...
        log.info({ uid, route: "register/options", ms: Date.now() - started, passkeys: passkeys.length }, "registration options ok");
        return res.json({ ok: true, options });
    }
    catch (err) {
        log.error({ err, route: "register/options", ms: Date.now() - started }, "register/options failed");
        logWebauthnEvent("passkey_error", { stage: "register_options", error: err?.message });
        await (0, alert_1.alertEvent)("passkey_error", { stage: "register_options", message: String(err?.message || "") });
        const isTimeout = err?.message?.toString().includes("timeout");
        return res
            .status(isTimeout ? 504 : 500)
            .json({ ok: false, error: err?.message || "internal error" });
...
router.post("/register/verify", verifyAuth_1.verifyAuth, async (req, res) => {
    try {
        const uid = req.user?.uid || req.uid;
        if (!uid)
            return res.status(401).json({ ok: false, error: "unauthorized" });
        ...
        const now = nowIso();
        const db = (0, firestore_1.getFirestore)();
        const label = typeof req.body?.client?.label === "string"
            ? req.body.client.label.slice(0, 120)
            : undefined;
        const userAgent = typeof req.body?.client?.userAgent === "string"
...
router.post("/authenticate/options", async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ ok: false, error: "no user for phone" });
        ...
        const options = await withTimeout((0, server_1.generateAuthenticationOptions)({
            rpID: rpIdForRequest(req),
            timeout: 60000,
            userVerification: "preferred",
            allowCredentials: passkeys.map(toCredentialDescriptor),
        }), timeoutMs, "generateAuthenticationOptions");
        ...
        await (0, alert_1.alertEvent)("passkey_error", { stage: "authenticate_options", message: msg });
...
router.post("/authenticate/verify", async (req, res) => {
    try {
        ...
        const db = (0, firestore_1.getFirestore)();
        const passkeySnap = await withDbTimeout(db
            .collection("users")
            .doc(user.uid)
            .collection(PASSKEY_COLLECTION)
            .doc(credentialId)
            .get(), "getPasskeyForAuth");
        if (!passkeySnap.exists) {
            return res.status(404).json({ ok: false, error: "passkey not registered" });
        }
        const stored = passkeySnap.data();
        const verification = await withTimeout((0, server_1.verifyAuthenticationResponse)({
            response: credential,
            expectedChallenge: pending.challenge,
            expectedOrigin: originsForRequest(req),
            expectedRPID: rpIdForRequest(req),
            ...
            requireUserVerification: true,
        }), timeoutMs, "verifyAuthenticationResponse");
        if (!verification.verified) {
            return res.status(400).json({ ok: false, error: "authentication failed" });
        }
        const { newCounter } = verification.authenticationInfo;
        await withDbTimeout(passkeySnap.ref.set({ counter: newCounter, last_used_at: nowIso() }, { merge: true }), "updatePasskeyCounter");
        await clearChallenge(user.uid);
        const adminAuth = (0, auth_1.getAuth)();
        const token = await withTimeout(adminAuth.createCustomToken(user.uid, { phone }), timeoutMs, "createCustomToken");
        ...
        await (0, alert_1.alertEvent)("passkey_error", { stage: "authenticate_verify", message: msg });
