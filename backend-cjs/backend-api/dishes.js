...
const log = logger_1.default.child({ module: "dishes" });
...
function normalizeDish(it, idx) {
    ...
    const restaurantName = it.restaurantName || it.restaurant_name || it.store_name || it.restaurant || "";
    ...
    const tags = [];
    if (it.bestseller || it.isBestseller)
        tags.push("bestseller");
    if (it.top_ordered || it.topOrdered)
        tags.push("top_ordered");
    if (it.pureVeg || it.isVeg)
        tags.push("pure_veg");
    const etaMins = typeof it.sla === "number"
        ? it.sla
...
router.get("/home", async (req, res, next) => {
    try {
        ...
        else if (!pincode && !address && !city && fallbackPincode)
            params.set("pincode", fallbackPincode);
