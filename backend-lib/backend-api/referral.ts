...
import { addCoins } from "./user/coins";
...
router.post("/create", async (req: Request, res: Response) => {
  try {
    ...
    if (!code) return res.status(500).json({ ok: false, error: "could not allocate code" });
...
router.post("/redeem", async (req: Request, res: Response) => {
  try {
    ...
    if (!codeDoc.exists) return res.status(404).json({ ok: false, error: "invalid code" });
    ...
    if (uses >= limit) return res.status(400).json({ ok: false, error: "code exhausted" });
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (curUses >= curLimit) throw new Error("code exhausted");
      tx.update(codeSnap.ref, { uses: FieldValue.increment(1), last_use_at: new Date().toISOString() });
