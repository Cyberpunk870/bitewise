...
async function computeAggregates(uid: string) {
  ...
  snap.forEach((doc) => {
    ...
    if (ts >= sinceMonth) monthlyEarned += amt;
    if (ts >= sinceDay) dailyEarned += amt;
    const reason = String(data?.reason || "").toLowerCase();
    const isRedeem = reason.startsWith("redeem") || Boolean((data?.meta as any)?.redeem === true);
    if (isRedeem && ts >= sinceMonth) redeemedThisMonth += Math.max(0, amt);
  });
...
export async function getCoinsSummary(uid: string) {
  const db = getFirestore();
  const userRef = db.collection("users").doc(uid);
  const userSnap = await userRef.get();
  const total_coins = userSnap.exists ? Number(userSnap.data()?.total_coins || 0) : 0;
  const { dailyEarned, monthlyEarned, dailyRemaining, monthlyRemaining, redeemableCap, redeemableRemaining } =
    await computeAggregates(uid);
  return {
    ok: true,
    total_coins,
    dailyEarned,
